...
- HUD surface/text theming should flow through `ui/src/uiConfig/sections/bottomDock.ts` plus `ui/src/uiDock.ts`; timer, pause toggle, dice borders, chat input, and chat popup now consume these shared dock tokens instead of local hardcoded colors.
