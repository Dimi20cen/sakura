...
- Trade editor controls now use a dedicated 3-button action rail in `ui/src/trade.ts`: top submits a bank trade, middle submits a co-player trade offer, and bottom clears/cancels the current draft. The bank button is disabled unless the local viewer is the current player and the current draft satisfies bank-trade ratio/exchange validity; counter-offer drafts never expose bank submission.
...
- Live trade-offer cards in `ui/src/trade.ts` also use the same light board-style chrome as a single stacked card, but the rows are viewer-relative for active players: top shows cards the local viewer receives, bottom shows cards the local viewer gives. Spectator/current-player-neutral views continue to read from the current-player perspective.
- Live trade-offer right-side actions are role-specific in `ui/src/trade.ts`: incoming offers to non-creator recipients show `edit + reject + accept`, outgoing offers created by the local player show per-player response status chips (`accepted`, `declined`, `pending`) plus `cancel`, and accepted chips are clickable so the offer creator can finalize with that responder. Current-player counter-offer views show explicit `accept + reject` actions instead of relying only on the presence/status strip.
- Live trade-offer left markers are role-aware for clarity: the receive row shows either the group icon (for outgoing offers awaiting another player) or the offering player's avatar, while the give row shows the local player's avatar for active participants and the current-player avatar for spectators. Hover tooltips on markers/actions explain transfer direction and button intent.
